...
# Simulation using spatial coordinates provided in 
# TIEDENSITY.RData.
...
N = seq(from=100, to=500, by=2)
a = seq(from=1.1, to=1.5, by=0.05) # clustering exponent. the Higher the exponent, more difficult the conection at higher distances
...
for (j in 1:length(a)) {
  ...
  for (i in seq_along(1:length(N))) {
    ...
    C <- genStatialDistribution(df = df, alpha = alpha )
    g <- genSpatialNet(df = df, number_of_humans = N[i], C = C, alpha = alpha )
...
write.csv(super_container,'ER_spatial_simulation240120.csv')
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
...
super_container <- read.csv("ER_spatial_simulation240120.csv")
